...
int evaluate(struct nodeTypeTag *p){
	...
	switch(p->nodeClass){
		...
		case FUNC:
					freePushLevel();
					fprintf(fp,"MOV R0, R0//\tPushing All Used Registers\n");
	...
	switch(p->nodeClass){
		...
		case FUNC :  
					...
					while(arguments && fArgExps){
						...
						else if((result==1)&&(strcmp(arguments->type->name,"int") == 0)){
							result = result;
						}
						else{
		...
		case ID       : 
						if(getLocalVariableBinding(p->id.name,currLocalTable) != 0){
							...
							if(strcmp(p->id.LsymbolPointer->type->name, "int") == 0)
								return 1;
							...
							if(analyse(p->id.expressionNode) == 1){
								if(strcmp(p->id.GsymbolPointer->type->name, "int") == 0)
									return 1;
		...
		case OPERATION:
				switch(p->opr.oper){
					...
					case '=' : 
								if(analyse(&p->opr.op[0]) == analyse(&p->opr.op[1]))
									return 1;
								else{
					...
					case 'R' : 
								if(getLocalVariableBinding(p->opr.op[0].id.name,currLocalTable) != 0){
									...
									if(strcmp(p->opr.op[0].id.LsymbolPointer->type->name, "int") == 0)
										return 1;
									else
										printf("line %d : Invalid Read Operation \n",p->lineNumber);
								}
								else if(strcmp(p->opr.op[0].id.GsymbolPointer->type->name, "int") == 0)
									return 1;
								return 0;
					case 'W' : 
					...
					case '^' :
								...
								else if((analyse(&(p->opr.op[0])) == 1) && (strcmp(currFunction->type->name,"int") == 0)){
									return 1;
								}
								else{
